...
Implement authenticated wishlist.

## Requirements

Add/remove/list/check wishlist. Guest access shows login-required UI. Update header state. Implement a small Medusa custom module/domain if the chosen Medusa setup does not provide the exact functionality.

## Acceptance criteria

[ ] Guest is blocked
[ ] Authenticated add/remove/list works
[ ] Empty state works
[ ] Errors rollback or recover cleanly

## Explicit non-goals

Do not build guest wishlist.
