    ...
    sections: [
      ...
      },
      {
        ...
        content: `
          ...
          <p>Founded in 2026, EcomFashion was born out of a deep reverence for India’s rich textile heritage. We collaborate directly with artisanal weaving clusters in Rajasthan, Gujarat, and Varanasi to bring you authentic, breathable fabrics crafted with integrity.</p>
          <h3>Sustainable & Ethical Sourcing</h3>
          <p>Every piece in our collection is crafted with zero-waste cutting techniques, natural botanical dyes, and fair-wage artisan partnerships.</p>
        `,
      },
      {
        __component: 'sections.promotional-cta',
        title: 'Experience The Craft',
        description: 'Join over 100,000 fashion enthusiasts who choose authentic artisanal wear with fast express delivery.',
    ...
    sections: [
      {
        ...
        content: `
          ...
          <p>At EcomFashion, accessible from ecomfashion.com, one of our main priorities is the privacy of our visitors. This Privacy Policy document contains types of information that is collected and recorded by EcomFashion and how we use it.</p>
          <h2>1. Information We Collect</h2>
          <p>When you place an order, create an account, or subscribe to our newsletter, we collect personal information including name, email, phone number, shipping address, and payment preferences.</p>
          <h2>2. How We Use Your Information</h2>
          <p>We use the information we collect to process orders, manage deliveries, send order status updates, and improve customer experience.</p>
          <h2>3. Payment Security & COD</h2>
          <p>We do not store your credit card numbers or UPI PINs. All payment transactions are encrypted through PCI-DSS compliant payment gateways.</p>
        `,
      },
    ],
  },
...
async function seedTask10Pages() {
  ...
  console.log('Strapi loaded for Task 10 pages seeding.');

  for (const pageData of TASK_10_PAGES) {
    try {
      ...
      if (existing) {
        ...
        console.log(`✓ Updated and published "${pageData.title}" (${pageData.slug})`);
      } else {
        const created = await strapi.documents('api::page.page').create({
          data: pageData,
          status: 'published',
        });
        console.log(`✓ Created and published "${pageData.title}" (${pageData.slug}) [${created.documentId}]`);
      }
    } catch (err) {
      console.error(`Error seeding page ${pageData.slug}:`, err);
    }
  }

  await strapi.destroy();
  console.log('✓ Task 10 pages seeding complete.');
}

seedTask10Pages().catch(console.error);
